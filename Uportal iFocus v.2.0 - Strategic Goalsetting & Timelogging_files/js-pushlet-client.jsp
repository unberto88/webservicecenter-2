 ...
 *
 * $Id: js-pushlet-client.js,v 1.1 2006/05/06 00:03:18 justb Exp $
 */
var flag = false;
...
function p_join() {
  ...
  if (pushletNet) {
    ...
    pushletNet.setControlURI(pushletURI + '?p_event=join&p_user='+p_user);
  } else {
...
function p_join_listen(subject,name, mode) {
  p_join(name);
...
function p_leave() {
  ...
  var uri = pushletURI + '?p_event=leave&p_subject=' + listenSubject;
...
function _push(args) {
  ...
  } else if (eventType == 'refresh-users') {
    _setStatus('refresh user list' + event.get('p_user_list'));
    _doCallback(event, window.onRefreshUserList);
...
function _initURIs() {
  ...
  pushletNetURI = getWebRoot() + 'modules/timelog/js/js-pushlet-net.html';
}
...
function _sendControlURI(uri) {
  if (controlQueue.isFull()) {
    ...
    alert('Currently network is very slow, please wait or use faster connection');
    // no sense going on
    return;
  }
